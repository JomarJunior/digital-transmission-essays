...
class CalculateNoiseVarianceStep(BaseSimulationStep):
    ...
    def execute(self, data: ChainDataType) -> ChainDataType:
        ...
        # Here we assume the channel has a method to get average gain
...
class CalculateSNRStep(BaseSimulationStep):
    ...
    def execute(self, data: ChainDataType) -> ChainDataType:
        ...
        # Estimate noise power based on channel model
        minimum_length = min(len(received_signal[0]), len(modulated_signal[0]))
        received_signal = received_signal[:, :minimum_length]
        modulated_signal = modulated_signal[:, :minimum_length]

        noise_power = np.mean(np.abs(received_signal - modulated_signal) ** 2)
...
class Simulation:
    ...
    def setup_chain(self) -> None:
        ...
        constellation_demapping_step.set_next(plot_constellation_step)

        # ------------------------------------------------------------------------------------------
        # Calculate BER Step
        # ------------------------------------------------------------------------------------------
        calculate_ber_step = CalculateBERStep()
        plot_constellation_step.set_next(calculate_ber_step)
